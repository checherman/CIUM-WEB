/**
 * @ngdoc object
 * @name Transaccion.PCCtrl
 * @description
 * Complemento del controlador CrudCtrl  para tareas especificas en Evaluacion PC
 */
(function() {
  "use strict";
  angular
    .module("App")
    .controller("PCCtrl", function(
      $rootScope,
      $translate,
      $localStorage,
      $stateParams,
      ...
      $scope.permisos = $localStorage.cium.permisos;
      ...
      /**
       * @ngdoc method
       * @name Transaccion.PCCtrl#abrirFicha
       * @methodOf Transaccion.PCCtrl
       *
       * @description
      ...
      /**
       * @ngdoc method
       * @name Transaccion.PCCtrl#CluesChange
       * @methodOf Transaccion.PCCtrl
       *
       * @description
        ...
        CrudDataApi.ver(
          ...
          function(data) {
            ...
            if (data.status == 200) {
              ...
              $scope.dato.macrored = data.data.macrored;
              $scope.dato.mesored = data.data.mesored;
              $scope.dato.microred = data.data.microred;
            } else {
              errorFlash.error(data);
            }
            $scope.cargando = false;
      ...
      /**
       * @ngdoc method
       * @name Transaccion.PCCtrl#cargarCatalogo
       * @methodOf Transaccion.PCCtrl
       *
       * @description
       * Con esta funcion listamos cualquier catalogo que especifiquemos con la url
       * @param {string} url url del plataforma comunitaria a listar
      ...
      $scope.cargarCat = function(url, modelo) {
        ...
        listaOpcion.options(url).then(function(response) {
          if (response) {
            $scope[modelo] = response.data.data;
          }
        });
      };
      ...
      $scope.cargarCatalogo = function(url) {
        $scope.cargando = true;
        if (!angular.isUndefined($scope.dato.microred))
          url = url + "&microred=" + $scope.dato.microred;
      ...
      $scope.resetear_meso = function() {
        CrudDataApi.lista(
          "/ActualizarMacroRedes",
          function(data) {
            data = data.data;
      ...
      $scope.init = function(buscar, columna) {
        ...
        var macrored = $scope.macrored;
        var mesored = $scope.mesored;
        var microred = $scope.microred;
        ...
        CrudDataApi.lista(
          ...
          function(data) {
            ...
            if (!data.priorizadas && $scope.permisos.indexOf("ComunidadPriorizadaController.store") > -1) {
              var scope = $scope;
              var confirm = $mdDialog
                .confirm()
                .title($translate.instant("NO_EXISTE_PRIORIZADAS"))
                .textContent($translate.instant("DESEA_CREAR_PRIORIZADAS"))
                .ariaLabel("priorizada")
                .ok("Aceptar")
                .cancel("Cancelar");

              $mdDialog.show(confirm).then(function() {
                scope.ir('/comunidad-priorizada/nuevo');
                }, function() {});
            }
            $scope.cargando = false;
          },
      ...
      /**
       * @ngdoc method
       * @name Transaccion.PCCtrl#abrirEvaluacionFicha
       * @methodOf Transaccion.PCCtrl
       *
       * @description
      ...
      $scope.abrirEvaluacionFicha = function(ev) {
        ...
        $scope.editDialog.show({
          ...
          scope: $scope.$new(),
          templateUrl:
            "src/transaccion/evaluacionPC/views/evaluacionFicha.html",
          clickOutsideToClose: true
        });
      };
      ...
      $scope.generarImpreso = function() {
        $localStorage.cium.pc = {};
        $localStorage.cium.pc.imprimir = {};
        $localStorage.cium.pc.imprimir.um = $scope.dato;
        $localStorage.cium.pc.imprimir.indicadores = $scope.tempIndicador;
        $location.path("evaluacion-p-c/evaluacionImpresa");
      };
      $scope.vistaImpreso = function() {
        $scope.dato = $localStorage.cium.pc.imprimir.um;
        $scope.indicadores = [];
        var cone = $scope.dato.idCone;
        angular.forEach($localStorage.cium.pc.imprimir.indicadores, function(
          val,
          key
        ) {
          CrudDataApi.lista(
            "/CriterioEvaluacionPCImprimir/" + cone + "/" + val,
            function(data) {
              data = data.data;
              if (data.status == 200) {
      ...
      //Ver. Muestra el detalle del id de la plataforma comunitaria
      ...
      //Modificar. Actualiza la evaluacion de la plataforma comunitaria con los datos que envia el usuario
      ...
      //Borrar. Elimina la evaluacion de la plataforma comunitaria del parametro id
      ...
      /**
       * @ngdoc method
       * @name Transaccion.PCCtrl#cargarCriteriosVer
       * @methodOf Transaccion.PCCtrl
       *
       * @description
      ...
      $scope.cargarCriteriosVer = function() {
        ...
        $scope.cargando2 = true;
        $http.get(URLS.BASE_API + "/EvaluacionPCCriterio/" + idev).then(
          function(data) {
            data = data.data;
            if (data.status == "407") $window.location = "acceso";
      ...
      /**
       * @ngdoc method
       * @name Transaccion.PCCtrl#acciones
       * @methodOf Transaccion.PCCtrl
       *
       * @description
      ...
      /**
       * @ngdoc method
       * @name Transaccion.PCCtrl#verificarCambios
       * @methodOf Transaccion.PCCtrl
       *
       * @description
      ...
      /**
       * @ngdoc method
       * @name Transaccion.PCCtrl#cargarCriterios
       * @methodOf Transaccion.PCCtrl
       *
       * @description
      ...
      $scope.cargarCriterios = function(
        ...
        var microred = $scope.dato.microred;
              ...
              function(data) {
                ...
                if (data.status == 200) {
                  ...
                  if (!angular.isUndefined(nombre)) {
                    ...
                    $scope.informacion[codigo]["promedioGeneral"] =
                      (totalAprobado / totalCriterio) * 100;
      ...
      /**
       * @ngdoc method
       * @name Transaccion.PCCtrl#estadistica
       * @methodOf Transaccion.PCCtrl
       *
       * @description
      ...
      /**
       * @ngdoc method
       * @name Transaccion.PCCtrl#aprobar
       * @methodOf Transaccion.PCCtrl
       *
       * @description
      ...
      /**
       * @ngdoc method
       * @name Transaccion.PCCtrl#cerrar
       * @methodOf Transaccion.PCCtrl
       *
       * @description
       * Cerrar evaluación
      ...
      //Borrar. Elimina el PC del parametro id
      ...
      $scope.abrirFicha = function(ev) {
        ...
        $scope.editDialog.show({
          ...
          scope: $scope.$new(),
          templateUrl: "src/transaccion/evaluacionPC/views/ficha.html",
          clickOutsideToClose: true
        });
      };
      ...
      $scope.pdf = function() {
        ...
        json.header = "";
        json.footer = "";
        json.nombre = "plataforma_comunitaria_" + $scope.dato.id;
        if (json) {
          $scope.cargando = true;
          CrudDataApi.crear(
            "/html-pdf",
