...
(function() {
  ...
  angular
    .module("App")
    .controller("ModuloCtrl", function(
      $rootScope,
      $translate,
      $scope,
      $localStorage,
      ...
      $scope.datos = [];
      $scope.dato = {};
      $scope.dato.es_super = 0;

      // regresa los metodos por default al modelo
      $scope.defaultMetodo = function() {
        $scope.dato.metodos = [
          {
            nombre: "Guardar",
            recurso: "store",
          ...
          { nombre: "Listar", recurso: "index", metodo: "get", es_super: 0 },
          { nombre: "Ver", recurso: "show", metodo: "get", es_super: 0 }
        ];
      };
      $scope.defaultMetodo();
      // limpia todo el modelo
      $scope.quitarAllMetodo = function() {
        $scope.dato.metodos = [];
      };
      // Quita el elemento de la lista metodos
      $scope.quitarMetodo = function(index) {
        $scope.dato.metodos.splice(index, 1);
      };
      // Agrega una nueva linea de captura para los metodos
      ...
      $scope.padreSelected = function() {
        setTimeout(function() {
          $scope.$apply(function() {
            $scope.dato.sis_modulos_id = $scope.dato["sis_modulos_id"];
          });
        }, 2000);
      };
